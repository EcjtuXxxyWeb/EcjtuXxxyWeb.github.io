...
var shuzi=myDate.getSeconds(); 
	switch(shuzi%3){
		case 0: $(".infobg").css({background:'url(/page/main70/images/infobg2.jpg) no-repeat'});break;
		case 1: $(".infobg").css({background:'url(/page/main70/images/infobg3.jpg) no-repeat'});break;
		case 2: $(".infobg").css({background:'url(/page/main70/images/infobg4.jpg) no-repeat'});break;
		}
	/////
...
function SetHome(obj){ 
	try{ 
		obj.style.behavior='url(#default#homepage)';obj.setHomePage('http://www.xxxy.ecjtu.jx.cn/'); 
		} 
	catch(e){ 
		if(window.netscape) { 
						try { 
	...
	prefs.setCharPref('browser.startup.homepage','http://www.xxxy.ecjtu.jx.cn/'); 
	}else{ 
			alert("您的浏览器不支持，请按照下面步骤操作：\n1.打开浏览器设置。\n2.点击设置网页。\n3.输入："+'http://www.xxxy.ecjtu.jx.cn/'+"点击确定。"); 
			} 
		} 
	} 
function shoucang(){ 
	try{ 
		window.external.addFavorite('http://www.xxxy.ecjtu.jx.cn/','信息工程学院'); 
		} 
	catch (e){ 
			try{ 
				window.sidebar.addPanel('http://www.xxxy.ecjtu.jx.cn/','信息工程学院', ""); 
				} 
	catch (e){ 
